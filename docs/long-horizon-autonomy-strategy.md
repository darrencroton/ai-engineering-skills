...
It did not work, but not for the reason it looks like. The failure was that making *every* acceptance decision deterministic required machinery that grew without bound: failure-pattern taxonomies, retry classifiers and streak controls, dual state copies and reconciliation, and closed schemas validating fields the models had to report perfectly. The overhead of avoiding trusting any model judgement eventually cost more than the work it supervised, and — the cruel part — it failed hardest on exactly the weak models it was built to protect.
