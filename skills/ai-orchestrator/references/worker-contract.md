...
When `required_skills` contains `drift-audit` or `code-review`, the rendered worker prompt requires exactly one final `MC_AUDIT_VERDICT: PASS | PASS WITH RISKS | FAIL | BLOCKED` line. The helper parses that line after process exit and stores the result in the worker status as `skill_verdicts`. Missing or duplicate sentinels record a null verdict; they never default to pass. MC's opt-in gate accepts only the latest completed exact `PASS` for each required audit, so a later adverse retry supersedes an earlier pass.
