...
- **Master Controller (MC)** — the deterministic supervisor. Owns run state, launches exactly one developer harness per slice, and verifies every claim against local evidence: it recomputes the highest-risk gates itself (changed files against the frozen authorized surface, commit ancestry, clean worktree) and evidence-checks the rest (see "Safety Rules" for the exact trust boundary). Drives the bounded self-correcting repair loop by surfacing gate violations back into the live developer session. Holds sole authority to hard-stop for a human when a threshold is crossed (repair budget exhausted, integrity breach, approval-gated slice). MC never writes slice code and never delegates to a reviewer itself.
- **Slice Developer** — the harness in tmux. Executes one slice under MC's supervision by applying `scoped-implementation` → validation → `drift-audit` → `code-review` → `commit`, self-correcting within its own run first, and requesting read-only investigation, evidence, drift audit, or code review from Reviewers when useful. Reports a structured result but holds no final authority; MC verifies it. On an MC-surfaced correction it fixes the specific gap using the context it already built. It may not expand scope, push, deploy, or perform external side effects. Standalone `orchestrator` uses the same Developer role without MC above it.
- **Reviewer** — a read-only evidence provider the Developer requests through `orchestrator`'s semantic policy/request launcher. It may investigate, gather evidence, perform drift audit, and perform code review. It never edits, runs mutation-prone tests, performs Git/GitHub mutations, owns a gate, commits, or re-delegates. MC writes Reviewer policy; `orchestrator` validates the request, embeds Reviewer/skill instructions, composes harness flags, and launches; MC verifies the normalized contract and outcome.
...
2. **Initialize** - create the auditable `.ai-mc/runs/<timestamp>/run.json` mirror in the target repo and update `.ai-mc/current`. Immediately before the first harness launch, MC also creates a controller-owned state copy outside the worktree; harness prompts and environments never receive that control path.
...
5. **Capture artifacts** - preserve prompt, transcript or pane capture, git status, diff, validation summary, drift audit, code review, commit data, operational events when present, and structured developer result.
6. **Verify gates** - independently compare the Developer result to git state, plan authorization, validation, drift audit verdict, exact-`PASS` review verdict, residual-finding ledger, and commit state. Reviewer independence is preferred but degradable on default slices: Developer self-audit remains valid and is reported explicitly. Only when a slice opts in with `Independent audit required: yes` does MC require every configured Reviewer tool to launch through a validated semantic contract matching the immutable schema-v2 `reviewer-policy.json`, use the required model/effort, normalize to `reviewer` / `read-only`, run in the target repository, complete successfully, and emit helper-recorded `PASS` for separate `drift-audit` and `code-review` contracts. Every terminal slice entry records MC-derived provenance for both audits, including the successful Reviewer tool/label or Developer fallback context. Every non-pass outcome is classified with a stable failure signature as either `repairable` (a fixable gap — validation, drift, review, residual-ledger/result shape, opt-in Reviewer evidence, unauthorized files, changed-files bookkeeping, malformed result, missing commit, dirty worktree) or terminal `needs-human` (an integrity/trust breach — HEAD did not advance or is not descended from the slice start, or the Developer worked the wrong slice).
7. **Repair, advance, or stop** - on a repairable gate with budget remaining, MC archives the stale result, writes a targeted repair prompt naming the exact violation, surfaces it into the **live** developer session (preserving the context the developer already built), and re-verifies the complete, unrelaxed gate when a fresh result lands — a repair can never lower the bar, only grant another chance to clear the identical one. A signature-keyed circuit breaker bounds the loop: first failure earns an in-session correction, the same signature failing again earns exactly one fresh-session retry with the frozen prompt plus targeted repair context and the cumulative residual-findings ledger, and a third consecutive failure is terminal regardless of remaining budget (`policy.max_repair_attempts`, default 3). MC advances to the next slice only when every gate passes, and stops with a precise reason for human approval, integrity breach, exhausted budget, harness failure, or incomplete evidence.
...
Harness: codex unless I specify otherwise. (claude, copilot, and opencode are also validated MC developer harnesses — name one explicitly to use it.)
...
- For model-supervised runs on subscription harnesses, prefer an MC model on a different provider than the developer harness: if both share one subscription, a usage window stalls the supervisor and the supervised session at the same time.
...
MC decisions must not rely only on natural-language transcript interpretation. The developer must produce `developer-result.json`, and MC must verify claims against local evidence.

Trust boundary for audit verdicts: MC recomputes the highest-risk gate itself — the set of changed files against the authorized surface, using segment-aware matching so `*.md` does not cross directory boundaries — and it independently verifies commit ancestry, HEAD advancement, and a clean post-commit worktree. For locally produced drift-audit, code-review, validation, and residual-finding evidence, MC verifies the reported result shape plus required non-empty artifacts; it does not re-run the analysis or validation commands. Reviewer delegation is a degradable preference by default, not a gate. A slice can opt in to mechanical enforcement with `Independent audit required: yes`; only then must validated launch evidence cover every configured reviewer tool and two distinct purposes — one contract with exactly `required_skills: ["drift-audit"]` and another with exactly `required_skills: ["code-review"]` — with matching policy digest, slice/plan identity, model/effort, role/access, repository, positive subprocess pid, real out/err files, successful completion, and a helper-recorded `MC_AUDIT_VERDICT: PASS` for each skill. Missing, ambiguous, `PASS WITH RISKS`, `FAIL`, or `BLOCKED` reviewer verdicts do not pass and cannot be replaced by the developer's own claim. The helper records the sentinel after process exit; MC still does not semantically re-derive the reviewer's reasoning.

Controller state and terminal cleanup are fail-safe against accidental or model-driven worktree corruption. Once a harness is live, MC keeps an undisclosed controller-owned state copy outside the worktree and treats `.ai-mc/.../run.json` as an auditable mirror; a missing copy or mismatch stops normal commands. `stop-with-evidence` can recover from the controller copy, preserves the tampered mirror, captures evidence, stops every matching tmux session, and cancels every helper-tracked reviewer. If both JSON copies are unreadable, it still scans the run's session/artifact namespace and writes `emergency-stop/emergency-stop.json` without claiming the durable state was updated. Every terminal finalize/stop path scans all slice artifacts in the run and reaps their reviewer process groups, including stale reviewers from prior slices. This is defense against a model acting through the documented workspace, not an OS security boundary against a same-user process deliberately searching and rewriting arbitrary filesystem locations; profiles without a real filesystem sandbox cannot provide that stronger guarantee.
...
- `commit`: used by the developer for passing slices. MC verifies the commit state.
- `handoff`: records stop state and the next slice in Mode A. Mode B keeps continuation state in `run.json`, writes an MC-generated `slice-summary.md` for each terminal slice, and continuously refreshes `run-report.md` with gates, commits, blockers, next actions, and residual post-plan considerations.
...
python3 skills/master-controller/scripts/mc.py preflight --repo <path> --reviewer-tools <tool[,tool]> --allow-profile-command
...
python3 skills/master-controller/scripts/mc.py run-next --repo <path> --reviewer-tools <tool[,tool]> --allow-profile-command
python3 skills/master-controller/scripts/mc.py run-next --repo <path> --harness-model <model> --harness-effort <effort> --reviewer-tools <tool[,tool]> --reviewer-model <model> --reviewer-effort <effort> --allow-profile-command
python3 skills/master-controller/scripts/mc.py run --repo <path> --scope remaining --reviewer-tools <tool[,tool]> --allow-profile-command
python3 skills/master-controller/scripts/mc.py start-slice --repo <path> --reviewer-tools <tool[,tool]> --allow-profile-command
