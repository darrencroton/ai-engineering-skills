...
The active slice also persists the semantic launch configuration. Any automatic fresh-session recovery must compose from that snapshot rather than silently reverting to a harness default when the controlling invocation omits model/profile flags.
