...
- `references/developer-prompt.md`: the rendered slice Developer prompt and repair-prompt contracts. Any `{placeholder}` change must match `render_developer_prompt`/`render_repair_prompt` in `runtime.py`; literal braces must be escaped. The prompt embeds only `orchestrator/references/mc-slice-contract.md`, not the full transitive skill bundle.
...
- `scripts/mc_lib/`: the implementation, grouped by responsibility — `cli.py` (argument parsing), `commands.py` (command handlers), `plan.py` (plan parsing, eligibility, check-plan), `state.py` (run state I/O and audit provenance reports), `gates.py` (deterministic gate and normalized Reviewer-evidence verification), `git_ops.py` (git evidence and authorized-surface matching), `runner.py` (batch slice execution and the repair loop), `runtime.py` (slice runtime: prompts, environment, Reviewer policy, artifacts), `tmux_adapter.py` (session control and capture), `observation.py` (operational hints), `profiles.py` (mechanical harness profiles), `constants.py` (shared vocabulary), `models.py` (dataclasses).
...
- Fixes strengthen the layer that owns the problem. Do not move slice-developer responsibilities (self-correction, semantic verification of reviewer output) into MC; MC makes failures visible and enforces gates.
...
- Prompt contracts → `runtime.py` render functions + `references/developer-prompt.md` + `test_prompts.py`.
