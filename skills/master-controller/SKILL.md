...
7. When `finalize-slice` returns `"status": "repairable"` with `"mode": "in-session"`, the session is still alive and `current_slice` remains populated: deliver the returned `send_text` (a one-line pointer to the rendered `repair-prompt-repair-<round>.md`) into the live session with `send --text <send_text> --reason <reason>`, `wait` for a fresh result, then `finalize-slice` again. With `"mode": "fresh-session"` or `"mode": "relaunch"`, finalize has already relaunched a new session for the same slice itself — just `wait` and re-finalize. Budget and the same-signature circuit breaker are enforced deterministically from persisted `current_slice.repair`; finalize goes terminal on its own when they trip. The first slice freezes the complete run launch configuration (harness command/model/effort, worker tools/model/effort, and launch-policy flags). Every later slice and in-slice relaunch inherits it when flags are omitted; a conflicting explicit value fails closed and requires a new run. Model identity evidence is refreshed and slice-tagged for every launch, so an ambient-default launch cannot display a prior slice's verified identity.
...
Two narrow operational exceptions are deterministic and bounded. First, a `blocked` or `fail` orchestrator result is routed into the existing repair loop only when current-attempt evidence exactly matches a high-confidence, non-hard-stop `service_unavailable` / `transient` / `bounded-retry` signature; medium-confidence server or network errors remain terminal. Second, repeated idle observations spanning the configured ten-minute ceiling trigger the same signature-keyed repair machinery: one automatic continuation nudge, one fresh-session escalation if the same stall recurs, then the existing terminal circuit breaker. Neither path accepts work or relaxes a gate.
...
Trust boundary for audit verdicts: MC recomputes the highest-risk gate itself — the set of changed files against the authorized surface, using segment-aware matching so `*.md` does not cross directory boundaries — and it independently verifies commit ancestry, HEAD advancement, and a clean post-commit worktree. For locally produced drift-audit, code-review, validation, and residual-finding evidence, MC verifies the reported result shape plus required non-empty artifacts; it does not re-run the analysis or validation commands. Worker delegation is a degradable preference by default, not a gate. A slice can opt in to mechanical enforcement with `Independent audit required: yes`; only then must validated launch evidence cover every configured worker tool and two distinct purposes — one contract with exactly `required_skills: ["drift-audit"]` and another with exactly `required_skills: ["code-review"]` — with matching policy digest, slice/plan identity, model/effort, role/access, repository, positive subprocess pid, real out/err files, successful completion, and a helper-recorded `MC_AUDIT_VERDICT: PASS` for each skill. Missing, ambiguous, `PASS WITH RISKS`, `FAIL`, or `BLOCKED` worker verdicts do not pass and cannot be replaced by the orchestrator's own claim. The helper records the sentinel after process exit; MC still does not semantically re-derive the worker's reasoning.

Controller state and terminal cleanup are fail-safe against accidental or model-driven worktree corruption. Once a harness is live, MC keeps an undisclosed controller-owned state copy outside the worktree and treats `.ai-mc/.../run.json` as an auditable mirror; a missing copy or mismatch stops normal commands. `stop-with-evidence` can recover from the controller copy, preserves the tampered mirror, captures evidence, stops every matching tmux session, and cancels every helper-tracked worker. If both JSON copies are unreadable, it still scans the run's session/artifact namespace and writes `emergency-stop/emergency-stop.json` without claiming the durable state was updated. Every terminal finalize/stop path scans all slice artifacts in the run and reaps their worker process groups, including stale workers from prior slices. This is defense against a model acting through the documented workspace, not an OS security boundary against a same-user process deliberately searching and rewriting arbitrary filesystem locations; profiles without a real filesystem sandbox cannot provide that stronger guarantee.
